...
namespace Grocery_Server.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public AuthController(UserManager<User> userManager, JwtService jwtService, SignInManager<User> signInManager, IConfiguration config)
        {
        ...
        public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordDTO passwordCombo)
        {
            ...
            if (user == null)
                return NotFound();
            IdentityResult result = await _userManager.ChangePasswordAsync(user, passwordCombo.Password, passwordCombo.NewPassword);
            if (!result.Succeeded)
                return BadRequest();
            return Ok();
        }
