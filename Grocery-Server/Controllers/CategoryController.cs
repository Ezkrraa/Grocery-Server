...
namespace Grocery_Server.Controllers
{
    ...
    public class CategoryController : ControllerBase
    {
        ...
        {
            Group group = (await GetCurrentUser()).Group ?? throw new ArgumentException("how??? attribute not working wth");
        ...
        public async Task<IActionResult> Delete([FromQuery] Guid id)
        {
            ...
            GroceryCategory? foundCategory = group.CustomCategories.FirstOrDefault(cat => cat.Id == id);
        ...
        [HttpPatch("rename")]
        public async Task<IActionResult> Rename([FromBody] RenameInfoDTO renameInfo)
        {
            User user = await GetCurrentUser();
            Group group = GetGroup(user);

            GroceryCategory? foundCategory = group.CustomCategories.FirstOrDefault(cat => cat.Id == renameInfo.Id);
            if (foundCategory == null)
                return NotFound();
        ...
        {
            return await _userManager.GetUserAsync(User) ?? throw new Exception("Can't get a user even though it's being used in a [RequireGroup] class????");
        }
